...
- [x] T044 [P] Port `ingest/loaders.py` — every loader now takes an explicit `corpus_dir: Path` param instead of reading a module-level `REPO_ROOT` constant (constitution Principle X — no path lives inside the repo); `load_wikipedia`'s cache moves to `corpus_dir/kb/cache/` + test (new — never had one; covers `load_cards`, `load_wiki_md`, `load_epub`'s missing-file graceful-skip, `load_source`'s dispatch/ingest-false gating)
- [x] T045 [P] Port `ingest/chunkers.py` — `_SECTION_SPLITTER` (module-level in legacy, read `os.environ` at import time) is now a `@lru_cache`-wrapped lazy singleton reading `core.config.Settings` at first call, matching the lazy-singleton idiom used everywhere else in this port (`core/db.get_engine`, `memory/store.get_checkpointer`) — the alternative (reading `Settings` at module scope) would have broken the zero-env-var import guarantee + test (new)
- [x] T046 [P] Port `ingest/wiki_refine.py` (CLI tool, UNREF, kept as a tool outside the service) — unchanged, needed no adaptation (already took explicit file args, no repo-relative path assumptions); added missing type hints (`Tag` params) surfaced by this project's stricter `disallow_untyped_defs`
- [x] T047 Create `infra/corpus.yaml` — translated from the REAL `../w2w-corpus/kb/manifest.yaml` (read in full, not just the inventory's summary): 20 sources, 11 ingestable, matches exactly. Carries the manifest's actual field vocabulary forward unchanged (`status: have/fetch/distill/want-later`, `license: PD/CC-BY-SA/own/copyrighted/paid`) rather than the idealized schema `data-model.md` sketched before this file had been read closely — data-model.md corrected to match. `path` fields drop the legacy `data/` prefix (the corpus root IS `$CORPUS_LOCAL_DIR`, not a subdirectory of it). `want-later` entries (Black tie, Cocktail attire, Semi-formal wear, plus 4 others the real manifest also logs) carried forward unchanged. `sha256` is a documentation-only placeholder (`null`) — see T049's note on why the CLI never writes into this file.
- [x] T048 Port `ingest/build_kb.py` — reads `infra/corpus.yaml`, resolves every path via an explicit `corpus_dir` param; `build_vectorstore` now returns `ports.VectorStore` (surfaced a real pre-existing Protocol bug — see T050's note); `write_attributions`/`_print_report` (renamed `print_report`, no longer private — `ingest/cli.py` calls it) unchanged in logic; dropped the legacy `__main__` block entirely — `ingest/cli.py` is now the one CLI entry point (constitution: "never an HTTP endpoint," singular) + test (new — `_validate`, `ingest_all` against a fake manifest+corpus, the reference-only-leak guard in `print_report`)
- [x] T049 Create `backend/src/whattowear/ingest/cli.py` — idempotent-by-content-hash CLI entry point (FR-011). Hash state lives in `$CORPUS_LOCAL_DIR/.ingest_state.json`, **not** written back into `infra/corpus.yaml` — a person-authored, comment-rich file; PyYAML's `safe_dump` isn't comment-preserving, so writing computed hashes into it would silently strip the licence-policy documentation on the very first run. `data-model.md` updated with this correction. + test (new — `_hash_file`, state round-trip, `_current_hashes` against a fake manifest)
- [x] T050 Port `kb.py` (process-wide KB singleton) — uses `ingest.build_kb` + `ports.VectorStore`; `KnowledgeBase.vectorstore` is now typed `ports.VectorStore` instead of bare `object` — this is what finally made `ports.VectorStore`'s Protocol signature type-checkable end-to-end for the first time, and it caught a **real bug**: `similarity_search`'s `filter` param was typed `dict[str, Any] | None` in `ports.py` since Phase 1, but `retrieval/hybrid.py::_l1_semantic_filter` actually builds and passes a `qdrant_client.models.Filter` object, not a dict. Fixed in `ports.py` (type annotation only, no behavior change — the real `QdrantVectorStore.similarity_search` always accepted a `Filter`; only the Protocol's documentation of that shape was wrong). + test (new — `KnowledgeBase.by_layer`, `get_kb()`'s `CORPUS_LOCAL_DIR`-required error path)

**Checkpoint**: verified — `mypy src` is **fully clean, zero errors**, across all 57 source files — the first point in this port where every module (including `kb.py` itself) type-checks end-to-end. `ruff`/`ruff format` clean. All 6 new `ingest.*` + `kb` modules import with zero env vars (48/48 import-safety cases pass, ~273s). 39 new tests pass; full suite (excluding the slow subprocess import-safety file) is 397 passed.
...
- [x] T061 Final report delivered to the user in chat (per handoff §10). SC-006 confirmed: `git log --all --diff-filter=A --name-only` across every commit on this branch shows only `infra/corpus.yaml` ever staged under a corpus-adjacent path — no file under `$CORPUS_LOCAL_DIR` (`../../w2w-corpus`), no `.ingest_state.json`, ever added. Constitution Check (plan.md): 7 of 10 principles PASS, 3 (VII Contracts, VIII Visual truth, IX One codebase) N/A — no API/UI surface in this slice — zero unresolved violations, nothing skipped silently.
