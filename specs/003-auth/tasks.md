...
- [x] T001 Add `@supabase/supabase-js` and `@supabase/ssr` to `frontend/package.json`
      dependencies; `npm install` in `frontend/`
- [x] T002 [P] Add `pyjwt>=2.9.0,<3.0.0` to `backend/pyproject.toml` `dependencies`, and
      `pytest-mock>=3.14.0,<4.0.0` to its `dev` group; `uv sync` in `backend/`
- [x] T003 [P] Create `frontend/.env.example` with `NEXT_PUBLIC_SUPABASE_URL` and
      `NEXT_PUBLIC_SUPABASE_ANON_KEY` placeholders (values come from `npx supabase status`,
      never committed as real values)
- [x] T004 [P] Add `SUPABASE_URL` and `SUPABASE_JWT_AUD=authenticated` placeholders to
      `backend/.env.example`, matching the existing file's comment style
- [x] T005 [P] In `infra/supabase/config.toml`, raise `minimum_password_length` from `6` to
      `8` under `[auth]` (design-decisions §1.7 — the server must enforce what the client
      claims)
- [x] T006 [P] In `infra/supabase/config.toml`, add an `[auth.external.google]` block
...
- [x] T007 [P] Create `frontend/lib/supabase/client.ts` — browser Supabase client,
      `flowType: 'pkce'`, reading `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY`
- [x] T008 [P] Create `frontend/lib/supabase/server.ts` — Server Component / Route Handler
      Supabase client using `@supabase/ssr`'s `createServerClient`, reading/writing cookies
      via Next's `cookies()`
- [x] T009 Create `frontend/lib/supabase/middleware.ts` — the session-refresh helper
...
- [x] T010 Create `frontend/proxy.ts` (Next 16's current file convention — `middleware.ts` is
...
- [x] T011 [P] Create `frontend/components/auth/AuthShell.tsx` + `.module.css` — the shared
...
- [x] T012 [P] Create `frontend/lib/auth-validation.ts` — shared field validators
...
- [x] T013 Create `frontend/lib/auth-validation.test.ts` — unit tests for T012's own logic in
      isolation: no error before blur, error appears on blur, re-validates on every change
      once already errored (not before), each copy key maps to design-decisions §1.7's exact
      string. Depends on T012. (Closes the gap where this shared lib was previously only
      exercised indirectly through each form's component test.)
- [x] T014 Create `frontend/app/(auth)/layout.tsx` using `AuthShell` and the existing
...
- [x] T015 [P] [US1] Component test `frontend/components/auth/SignUpForm.test.tsx` —
...
- [x] T016 [US1] Create `frontend/components/auth/SignUpForm.tsx` + `.module.css` — `Input`
...
- [x] T017 [US1] Create `frontend/app/(auth)/signup/page.tsx` — mounts `SignUpForm`, promotes
      the wordmark to `<h1>` (handoff §5.1 — no `TopHeader` on this screen); depends on T014,
      T016
- [x] T018 [US1] On successful `signUp`, redirect client-side to `/recommend` (session cookie
...
- [x] T019 [P] [US2] Component test `frontend/components/auth/SignInForm.test.tsx` — renders
...
- [x] T020 [US2] Create `frontend/components/auth/SignInForm.tsx` + `.module.css` — mirrors
      `SignUpForm`'s shape minus the confirm field; depends on T012
- [x] T021 [P] [US2] Create `frontend/components/auth/GoogleButton.tsx` — visual only, takes
...
- [x] T022 [US2] Create `frontend/app/(auth)/signin/page.tsx` — mounts `SignInForm` +
      `GoogleButton` (onClick wired in US4), wordmark promoted to `<h1>`; also mount
      `GoogleButton` on `frontend/app/(auth)/signup/page.tsx` (T017) alongside `SignUpForm`,
      per FR-004's "both screens" requirement. Depends on T017, T020, T021.
...
- [x] T023 [P] [US5] Update `frontend/e2e/root-redirect.spec.ts` — two scenarios: signed-out
      `/` → `/signin`; signed-in `/` → `/recommend` (replaces the old single unconditional
      assertion, which tested feature 001's stub per FR-012)
- [x] T024 [P] [US5] Create `frontend/e2e/route-protection.spec.ts` — signed-out visiting
...
- [x] T025 [US5] Update `frontend/app/page.tsx` — replace feature 001's unconditional
...
- [x] T026 [US5] Add a sign-out `Button` to `frontend/app/(app)/profile/page.tsx`, calling
...
- [x] T027 [P] [US3] Component test `frontend/components/auth/ForgotPasswordForm.test.tsx` —
      submitting any email (registered or not) renders the same confirmation state in place,
      no route change
- [x] T028 [P] [US3] Component test `frontend/components/auth/ResetPasswordForm.test.tsx` —
...
- [x] T029 [US3] Create `frontend/components/auth/ForgotPasswordForm.tsx` + `.module.css` —
...
- [x] T030 [US3] Create `frontend/app/(auth)/forgot-password/page.tsx`; depends on T014, T029
- [x] T031 [US3] Create `frontend/components/auth/ResetPasswordForm.tsx` + `.module.css` —
      ...
      depends on T012
- [x] T032 [US3] Create `frontend/app/(auth)/reset-password/[token]/page.tsx` — validates the
      ...
      not linked from any in-app navigation (handoff trap #3). Depends on T031.
- [x] T033 [P] [US3] Create `frontend/e2e/password-reset.spec.ts` — request a reset, fetch the
      email from local Mailpit's HTTP API (`http://127.0.0.1:54324`, per quickstart.md),
      open its link, set a new password, confirm landing on `/signin` (not authenticated —
      FR-008), then explicitly assert **both** halves of SC-005: sign-in with the new
      password succeeds, and sign-in with the old password now fails. Needs a running local
      Supabase (research.md §5) — same live-stack caveat as the rest of the e2e suite.
...
- [x] T034 [P] [US4] Component test extension in `SignInForm.test.tsx`/`SignUpForm.test.tsx`
...
- [x] T035 [US4] Wire `GoogleButton`'s `onClick` in both `SignInForm` and `SignUpForm` to
      `supabase.auth.signInWithOAuth`; depends on T021
- [x] T036 [US4] Create `frontend/app/auth/callback/route.ts` — exchanges the auth code for a
...
- [x] T037 [P] [US6] `backend/tests/unit/test_auth.py` — mocked `_get_jwk_client`/`jwt.decode`
...
- [x] T038 [P] [US6] `backend/tests/integration/test_whoami.py` — `TestClient` hits
...
- [x] T039 [US6] Add `supabase_url: str` and `supabase_jwt_aud: str = "authenticated"` to
      `Settings` in `backend/src/whattowear/core/config.py`
- [x] T040 [US6] Create `backend/src/whattowear/auth.py` — `get_current_user_id` FastAPI
      ...
      `os.environ`/`load_dotenv` (research.md §2). Depends on T039.
- [x] T041 [US6] Create `backend/src/whattowear/api/v1/routes/whoami.py` — `WhoamiResponse`
      Pydantic model (`user_id: str`), `GET /api/v1/whoami` depending on
      `get_current_user_id`, per contracts/whoami.md. Depends on T040.
- [x] T042 [US6] Register the route in `backend/src/whattowear/main.py`
...
- [x] T043 [P] Create `frontend/e2e/auth-flow.spec.ts` — sign up → sign out → sign in → sign
      out, asserting each redirect (handoff §10's core Definition-of-Done scenario) — **and**
      explicitly assert FR-009/SC-004: after sign-in, reload the page and confirm the session
      still shows `/recommend` rather than redirecting to `/signin`
- [x] T044 [P] Keyboard-only pass across all four auth screens: confirm `:focus-visible` (not
      bare `:focus`) on every control including the password show/hide `IconButton`, and that
      focus lands on the wordmark `<h1>` after navigation (T014's `FocusOnNavigate` reuse)
- [x] T045 [P] Visual pass: every specified state (default/error/success/loading where
      applicable) of all four screens, both themes, at 320/768/1024/1440px (SC-007)
- [x] T046 Update `docs/ios-verification-backlog.md` — confirm or correct the four
      anticipated auth items (5–8) against what was actually built, add anything new (handoff
      §9)
- [x] T047 If `GoogleButton`'s treatment deviated from the standard four-colour mark, or any
      other genuine spec gap was found, record it in `docs/design-decisions.md` (handoff §11)
- [x] T048 Run `uv run pytest && uv run ruff check . && uv run mypy src && uv run lint-imports`
      in `backend/`, and `npm run lint && npm run typecheck && npm run build && npm test` in
      `frontend/` — all must be clean
- [x] T049 Run `quickstart.md`'s steps against a running local Supabase where this
...
- **US5 (Phase 5)**: depends on Foundational (its core mechanism, T010, already lives there);
  its own two tasks (T025 sign-out, T026 root redirect) have no dependency on US1–US4's forms,
  but T023/T024's e2e tests are most useful once US1/US2 exist to sign in with — recommended
  after Phase 3/4, not strictly required before
...
- T007/T008/T011/T012 (Foundational) are `[P]`; T009/T010/T013/T014 are sequential on top of
  them
...
- Commit after each task or small logical group, on `feat/003-auth`
- This environment cannot pull Docker images for `npx supabase start` (research.md §5) — tasks
  requiring a live local Supabase (parts of T043–T046, T049, T033, and the "valid token" case's
  real JWT in T038) are implemented and unit-tested here with mocks, but their live
  verification is a manual follow-up step, reported honestly rather than claimed
