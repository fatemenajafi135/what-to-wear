...
- [ ] T013 Create `frontend/lib/auth-validation.test.ts` — unit tests for T012's own logic in
...
- [ ] T014 Create `frontend/app/(auth)/layout.tsx` using `AuthShell` and the existing
...
- [ ] T015 [P] [US1] Component test `frontend/components/auth/SignUpForm.test.tsx` —
...
- [ ] T016 [US1] Create `frontend/components/auth/SignUpForm.tsx` + `.module.css` — `Input`
...
- [ ] T017 [US1] Create `frontend/app/(auth)/signup/page.tsx` — mounts `SignUpForm`, promotes
      the wordmark to `<h1>` (handoff §5.1 — no `TopHeader` on this screen); depends on T014,
      T016
- [ ] T018 [US1] On successful `signUp`, redirect client-side to `/recommend` (session cookie
...
- [ ] T019 [P] [US2] Component test `frontend/components/auth/SignInForm.test.tsx` — renders
...
- [ ] T020 [US2] Create `frontend/components/auth/SignInForm.tsx` + `.module.css` — mirrors
      `SignUpForm`'s shape minus the confirm field; depends on T012
- [ ] T021 [P] [US2] Create `frontend/components/auth/GoogleButton.tsx` — visual only, takes
...
- [ ] T022 [US2] Create `frontend/app/(auth)/signin/page.tsx` — mounts `SignInForm` +
...
- [ ] T023 [P] [US5] Update `frontend/e2e/root-redirect.spec.ts` — two scenarios: signed-out
      `/` → `/signin`; signed-in `/` → `/recommend` (replaces the old single unconditional
      assertion, which tested feature 001's stub per FR-012)
- [ ] T024 [P] [US5] Create `frontend/e2e/route-protection.spec.ts` — signed-out visiting
...
- [ ] T025 [US5] Update `frontend/app/page.tsx` — replace feature 001's unconditional
...
- [ ] T026 [US5] Add a sign-out `Button` to `frontend/app/(app)/profile/page.tsx`, calling
...
- [ ] T027 [P] [US3] Component test `frontend/components/auth/ForgotPasswordForm.test.tsx` —
      submitting any email (registered or not) renders the same confirmation state in place,
      no route change
- [ ] T028 [P] [US3] Component test `frontend/components/auth/ResetPasswordForm.test.tsx` —
...
- [ ] T029 [US3] Create `frontend/components/auth/ForgotPasswordForm.tsx` + `.module.css` —
...
- [ ] T030 [US3] Create `frontend/app/(auth)/forgot-password/page.tsx`; depends on T014, T029
- [ ] T031 [US3] Create `frontend/components/auth/ResetPasswordForm.tsx` + `.module.css` —
      ...
      depends on T012
- [ ] T032 [US3] Create `frontend/app/(auth)/reset-password/[token]/page.tsx` — validates the
...
- [ ] T033 [P] [US3] Create `frontend/e2e/password-reset.spec.ts` — request a reset, fetch the
...
- [ ] T034 [P] [US4] Component test extension in `SignInForm.test.tsx`/`SignUpForm.test.tsx`
...
- [ ] T035 [US4] Wire `GoogleButton`'s `onClick` in both `SignInForm` and `SignUpForm` to
      `supabase.auth.signInWithOAuth`; depends on T021
- [ ] T036 [US4] Create `frontend/app/auth/callback/route.ts` — exchanges the auth code for a
...
- [ ] T037 [P] [US6] `backend/tests/unit/test_auth.py` — mocked `_get_jwk_client`/`jwt.decode`
...
- [ ] T038 [P] [US6] `backend/tests/integration/test_whoami.py` — `TestClient` hits
...
- [ ] T039 [US6] Add `supabase_url: str` and `supabase_jwt_aud: str = "authenticated"` to
      `Settings` in `backend/src/whattowear/core/config.py`
- [ ] T040 [US6] Create `backend/src/whattowear/auth.py` — `get_current_user_id` FastAPI
...
- [ ] T041 [US6] Create `backend/src/whattowear/api/v1/routes/whoami.py` — `WhoamiResponse`
      Pydantic model (`user_id: str`), `GET /api/v1/whoami` depending on
      `get_current_user_id`, per contracts/whoami.md. Depends on T040.
- [ ] T042 [US6] Register the route in `backend/src/whattowear/main.py`
...
- [ ] T043 [P] Create `frontend/e2e/auth-flow.spec.ts` — sign up → sign out → sign in → sign
...
- [ ] T044 [P] Keyboard-only pass across all four auth screens: confirm `:focus-visible` (not
...
- [ ] T045 [P] Visual pass: every specified state (default/error/success/loading where
      applicable) of all four screens, both themes, at 320/768/1024/1440px (SC-007)
- [ ] T046 Update `docs/ios-verification-backlog.md` — confirm or correct the four
      anticipated auth items (5–8) against what was actually built, add anything new (handoff
      §9)
- [ ] T047 If `GoogleButton`'s treatment deviated from the standard four-colour mark, or any
      other genuine spec gap was found, record it in `docs/design-decisions.md` (handoff §11)
- [ ] T048 Run `uv run pytest && uv run ruff check . && uv run mypy src && uv run lint-imports`
      in `backend/`, and `npm run lint && npm run typecheck && npm run build && npm test` in
      `frontend/` — all must be clean
- [ ] T049 Run `quickstart.md`'s steps against a running local Supabase where this
