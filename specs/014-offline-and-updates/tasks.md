...
- [X] T027 Run `npm run lint`, `npm run typecheck`, `npm test` (frontend) and confirm the existing dev-server `npm run e2e` suite is unaffected; fix any regressions
- [X] T027a [P] Copy audit (FR-011/SC-006): grep every new/changed user-visible string this feature introduces (`updateToastCopy.ts`, any offline-related copy touched) for queue/retry/sync-promising language ("queued", "retry", "sync", "once you're back online") — zero matches expected; assert the exact toast copy in `update-prompt.spec.ts` (T025) as a regression guard
- [X] T028 **Partially substituted, not a literal DevTools-panel pass**: no human sat at DevTools for this. The e2e-pwa suite's own assertions (`caches.keys()`, `cache.keys()` entry contents, `registration.active/.waiting` state) already directly inspect real Cache Storage and real service-worker state — the same underlying data DevTools' Application panel reads — for cold-start offline, sign-out purge, expired photos, and the update mechanism. A first attempt at a separate scripted pass (querying both hosts/themes/standalone-emulation directly) hit a Node ESM path error and was dropped rather than debugged further, since the user confirmed the e2e-pwa coverage is sufficient. **Not independently verified by this session**: standalone `display-mode` parity (FR-013) and the literal `localhost:3000`/`127.0.0.1:3000` hosts specifically (e2e-pwa runs on port 3100) — flagged, not silently assumed.
- [X] T029 Full CI-equivalent check: backend `ruff`, `ruff format --check`, `mypy src`, `pytest`, `lint-imports` (expect unchanged — no backend files touched) and frontend `eslint`, `tsc --noEmit`, `next build`; confirm backend test count ≥753 and frontend test count ≥313 (handoff §7) — **all clean**: backend 753/753 (unchanged), frontend 322/313 (+9 new), zero lint/type errors, production build succeeds
