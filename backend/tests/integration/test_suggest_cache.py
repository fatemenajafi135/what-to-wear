...
def test_closet_edit_invalidates_the_cache(client, graph_call_spy):
    """Root-caused (superseding the "known intermittent failure" note this
    docstring used to carry): this test's `client` fixture only overrides
    `get_current_user_id`, not `get_session` (unlike `db_session`-isolated
    tests elsewhere), so its PATCH commits for real, permanently, against
    the live `EVAL_BASELINE_USER_ID` wardrobe. The old version patched a
    fixed literal pattern string -- a no-op, and therefore a false failure,
    on any run after the first one that happened to leave that exact value
    in place (confirmed by reading the row directly: it already carried
    that literal from a prior run). A unique value per invocation makes the
    edit a real content change every time, regardless of what a previous
    run left behind."""
    _suggest(client, occasion="office")
    assert graph_call_spy.call_count == 1

    item_id = _first_item_id(client)
    r = client.patch(f"/wardrobe/items/{item_id}", json={"pattern": f"edit-marker-{uuid.uuid4()}"})
    assert r.status_code == 200
