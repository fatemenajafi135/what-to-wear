...
class TestColorHarmonyScore:
    def test_high_contrast_pair_scores_higher_than_low_contrast_pair(self):
        high = color_harmony.score([_item("a", colors=["#000000"]), _item("b", "jeans", colors=["#ffffff"])], _ctx())
        low = color_harmony.score([_item("a", colors=["#808080"]), _item("b", "jeans", colors=["#808080"])], _ctx())
        assert high.value > low.value

    def test_reason_is_present(self):
        result = color_harmony.score([_item("a", colors=["#000000"]), _item("b", "jeans", colors=["#ffffff"])], _ctx())
