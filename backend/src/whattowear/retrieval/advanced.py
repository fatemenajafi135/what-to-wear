...
L4 (structured) and L1 (load-all) need no reranking; the semantic L3 layer is
where a cross-encoder rerank pays off. We over-fetch L3 candidates, then Cohere
rerank down to the final top-n.
"""
...
def retrieve(
    ...
    if "L1" in layers:
        res.l1 = hybrid.retrieve_l1(kb, ctx)
    if "L3" in layers:
        candidates = hybrid.retrieve_l3(kb, l3_query, k=first_stage_k)
