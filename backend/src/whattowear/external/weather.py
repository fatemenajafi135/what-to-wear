...
Geocodes a location, fetches current temperature + condition, and derives the
`temp_band` + `season` the L4 filter uses. Degrades gracefully: if the network
is unavailable, callers can pass `temp_c` directly and we still band it.
"""
...
def get_weather(location: str, timeout: float = 10.0) -> dict:
    ...
    Raises on network failure — callers that want a fallback should catch and
    supply temp_c manually (see context_assembler)."""
    geo = requests.get(
    ...
    return {
        ...
        "season": month_to_season(datetime.now(timezone.utc).month, southern=lat < 0),
    }
