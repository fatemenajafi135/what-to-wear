...
sources" — the demo's proof of grounding. Also exposes the pure verifiable
checks (owned-items, grounded-cites) that eval Harness B reuses.
"""
...
def owned_only(gen: GenOutput, wardrobe_ids: set[str]) -> tuple[bool, list[str]]:
    """True iff every item in every outfit is an owned wardrobe id. Returns the
    offending (hallucinated) ids. This is the highest-value grounding check."""
    offending = [
        i for outfit in gen.outfits for i in outfit.items if i not in wardrobe_ids
    ]
...
def render_text(result: OutfitResult) -> str:
    """Human-readable 'why + sources' rendering for the demo / test API."""
    lines = []
