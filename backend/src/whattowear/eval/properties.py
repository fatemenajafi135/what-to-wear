...
(coherence, occasion nuance) is scored by the LLM judge in the isolated evals
project. Expected values are PROPERTIES, not fixed outfits.
"""
...
def weather_appropriate(items: list[str], wardrobe: dict[str, WardrobeItem], expected: dict) -> bool:
    ...
    if expected.get("requires_outer"):
        ...
        if not has_warm:
            return False
    if "max_warmth" in expected:
        if any(it.warmth > expected["max_warmth"] for it in core):
            return False
    return True
