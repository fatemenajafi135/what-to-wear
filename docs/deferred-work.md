...
| 12 | **Feature 018 (photo-to-items — multi-garment detection, isolation) is parked, unmerged.** `feat/018-photo-to-items` on origin, PR #63 closed without merging. Built via Spec Kit, 819 backend tests passing, real-photo spot checks (6 photos) looked correct — then failed real usage across the board once actually tested end to end in the browser. `main` was never touched by this branch; nothing here regressed anything live. | PR #63 (closed, not merged) | Revisit as its own effort. The process gap that let this ship broken despite passing tests: verification never went through the real upload → browser → review-card path with a real account — only the detection function in isolation, and a handful of manual spot checks. Whoever picks this up should test that path FIRST, before any code, and keep testing it after every change — not just add more unit tests around the edges. |
| 13 | **No real segmentation/generative isolation provider is configured** (folded into #12 above — same parked branch). `WTW_SEGMENTATION_API_URL`/`_API_KEY` unset everywhere; the three-way comparison (#48) never ran against a real provider. | `design-decisions.md` §61/§62 (both PENDING) | Not started independently of #12 — do not resume this without also addressing why #12 failed real usage. |
