...
| 12 | **No real segmentation/generative isolation provider is configured.** `WTW_SEGMENTATION_API_URL`/`_API_KEY` are unset in every environment; `IsolationClient` no-ops gracefully (region-crop fallback), so nothing is broken, but no photo's image is ever actually isolated until an account exists. | `design-decisions.md` §62 (PENDING), `specs/018-photo-to-items/research.md` §5's "open item" | Needs a human to choose a real hosted background-removal vendor, provision an account/API key, and run T045's `--isolation-report` against T027's real fixture corpus to set the actual default strategy and hybrid thresholds — not a code gap. |
| 13 | **The photo-to-items fixture corpus is still the two pre-018 synthetic placeholders.** #46's accuracy claim and #48's strategy comparison are both structurally ready (`eval/vision_harness.py`) but unrun. | `specs/018-photo-to-items/tasks.md` T027/T028, `design-decisions.md` §61/§62 (both PENDING) | Needs a human with an actual closet and camera — sourcing real photos isn't something to fake or defer indefinitely; the two PENDING design-decisions entries name exactly what's owed once it exists. |
