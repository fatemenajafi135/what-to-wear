...
(() => {
  ...
  class DeckStage extends HTMLElement {
    ...
    _enableRail() {
      ...
      // (kept out of the way while presenting); only a stored '1' (from the
      // overlay's own toggle, or the TweaksPanel) shows it.
    ...
    _render() {
      ...
      overlay.innerHTML = `
        ...
        <button class="btn rail-toggle" type="button" aria-label="Toggle slide thumbnails" title="Toggle slide thumbnails" aria-pressed="false">
          <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="1.5" y="2.5" width="13" height="11" rx="1.5"/><line x1="6" y1="2.5" x2="6" y2="13.5"/></svg>
        </button>
      `;
    ...
    // Per-viewer show/hide for the thumbnail rail — shared by the overlay's
    // own toggle button and the TweaksPanel's postMessage contract (or any
    // author script) so both paths animate and persist identically.
    _setRailVisible(on) {
    ...
    _syncRailHidden() {
      ...
      // Keep the overlay toggle's pressed-state in sync — covers the
      // button's own click, the message-driven path, and the restored
      // localStorage preference on first render.
      if (this._overlay) {
        const btn = this._overlay.querySelector('.rail-toggle');
        if (btn) btn.setAttribute('aria-pressed', this._railVisible ? 'true' : 'false');
      }
    }
